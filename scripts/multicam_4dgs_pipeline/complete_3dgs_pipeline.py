...
class PathConfig:
    ...
    # 📁 用户配置路径（只需要修改这里）
    # ========================================
    ORIGIN_IMAGES = '../../data/firstimages'               # 原始图像目录（100台相机）
    OUTPUT_BASE_DIR = '../../data/superglue'  # 输出根目录
    ...
    @classmethod
    def configure(cls, origin_images, output_base_dir):
        """用命令行路径重建所有派生路径，避免修改源码中的硬编码目录。"""
        cls.ORIGIN_IMAGES = str(Path(origin_images).resolve())
        cls.OUTPUT_BASE_DIR = str(Path(output_base_dir).resolve())
        cls.COLMAP_SFM_DIR = f'{cls.OUTPUT_BASE_DIR}/colmap_sfm'
        cls.COLMAP_DATABASE = f'{cls.COLMAP_SFM_DIR}/database.db'
        cls.COLMAP_SPARSE = f'{cls.COLMAP_SFM_DIR}/sparse'
        cls.ESTIMATED_CALIB = f'{cls.COLMAP_SFM_DIR}/estimated_calib.json'
        cls.UNDISTORT_OUTPUT = f'{cls.OUTPUT_BASE_DIR}/undistorted'
        cls.UNDISTORTED_IMAGES = f'{cls.UNDISTORT_OUTPUT}/images_undistorted'
        cls.UNDISTORTED_CAMERAS = f'{cls.UNDISTORT_OUTPUT}/tool/cameras_undistorted.json'
        cls.HLOC_INPUT_DIR = f'{cls.OUTPUT_BASE_DIR}/colmap'
        cls.HLOC_OUTPUT_DIR = f'{cls.OUTPUT_BASE_DIR}/hloc_outputs'
        cls.HLOC_SPARSE_DIR = f'{cls.HLOC_OUTPUT_DIR}/sparse_multicam_text'
        cls.TRAINING_DATA_DIR = f'{cls.OUTPUT_BASE_DIR}/3dgs_training_data'
        cls.TRAINING_IMAGES = f'{cls.TRAINING_DATA_DIR}/images'
        cls.TRAINING_SPARSE = f'{cls.TRAINING_DATA_DIR}/sparse/0'
        cls.TRAINING_TRANSFORMS = f'{cls.TRAINING_DATA_DIR}/transforms.json'
...
def main():
    ...
    parser.add_argument('--images-dir', type=str, default=None,
                       help='用于标定的同一时刻多相机图像目录')
    parser.add_argument('--output-dir', type=str, default=None,
                       help='标定和首帧重建的输出根目录')
    
    args = parser.parse_args()

    def resolve_from_invocation(value):
        path = Path(value)
        return path.resolve() if path.is_absolute() else (invocation_dir / path).resolve()

    if args.images_dir is not None or args.output_dir is not None:
        origin_images = resolve_from_invocation(args.images_dir or PathConfig.ORIGIN_IMAGES)
        output_base = resolve_from_invocation(args.output_dir or PathConfig.OUTPUT_BASE_DIR)
        PathConfig.configure(origin_images, output_base)
